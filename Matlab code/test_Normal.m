...
for m = 1:total_test_k 
    for i=1:1000 
        MS_sim_sta(i,m) = simulation_static_normal(n,m,mu,sigma,F(i,:),C,R,D);
    end
...
for ii = 1:20 
    ...
    w_dyn = gamma * W_th;
    for i=1:1000 
        MS_sim_dyn(i,ii) = simulation_dynamic_normal(n,w_dyn,mu,sigma,F(i,:),C,R,D);
    end
...
for i=1:1000 %repeat 100 times
    MS_sim_dyn(i,21) = simulation_dynamic_normal(n,W_FO,mu,sigma,F(i,:),C,R,D);
end
