...
for m = 1:total_test_k 
    for i=1:1000 
        MS_sim_sta(i,m) = simulation_static_gamma(n,m,alpha,beta,F(i,:),C,R,D);
    end
...
for k=1:total_test_k
    MS_the_sta(k)=n*exp(lambda*R)*(1/lambda+D)*(exp(lambda*C)*((beta/(beta-lambda)))^(alpha*k)-1)/k;
end
x_static=(lambertw(-exp(-lambda*C-1))+1)/(alpha*log(beta/(beta-lambda)));
...
for ii = 1:20 
    ...
    w_dyn = gamma * W_th;
    for i=1:1000 
        MS_sim_dyn(i,ii) = simulation_dynamic_gamma(n,w_dyn,alpha,beta,F(i,:),C,R,D);
    end
...
for i=1:1000 %repeat 100 times
    MS_sim_dyn(i,21) = simulation_dynamic_gamma(n,W_FO,alpha,beta,F(i,:),C,R,D);
end
...
save P1e-2_alpha10_beta0_2.mat