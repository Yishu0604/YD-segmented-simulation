...
C = eta * alpha/beta; %fixed checkpoint time
...
for m = 1:total_test_k %checkpoint after m iterations
    for i=1:1000 %repeat 100 times
        T_sta(i,m) = simulation_static_gamma(n,m,eta,alpha,beta,F(i,:));
    end
    T_ave_sta(m)=sum(T_sta(:,m))/1000 %the average result
end
[Time_k_simulation_OPT,k_simulation_OPT]=min(T_ave_sta);

k_static_OPT = max((lambertw(-exp(-lambda*C-1))+1)/(alpha*log(beta/(beta-lambda))),1);
k_daly=max(1/E_X*sqrt(2*C/lambda),1);
% Time_k_static_OPT=T_ave_sta(round(k_static_OPT))
for k=1:total_test_k
    MS(k)=n*exp(lambda*R)*(1/lambda+D)*(exp(lambda*C)*((beta/(beta-lambda)))^(alpha*k)-1)/k;
end
...
E_elambdaX = (beta/(beta-lambda))^alpha;
...
for ii = 1:20 %checkpoint after m iterations
    ...
    for i=1:1000 %repeat 100 times
        T_dyn(i,ii) = simulation_dynamic_gamma(n,w_dyn,eta,alpha,beta,F(i,:));
    end
...
for i=1:1000 %repeat 100 times
    T_dyn(i,21) = simulation_dynamic_gamma(n,w_dyn,eta,alpha,beta,F(i,:));
end
...
xlswrite('data_figure2_gamma.csv',data,1,'D26')
save P1e-0_1_alpha50_beta1.mat